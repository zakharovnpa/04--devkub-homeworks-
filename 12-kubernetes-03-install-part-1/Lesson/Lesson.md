...
2. На каждой из хост-систем запускается одна нода кластера Kubernetis
  * На первой хост-системе - Control plane (мастер нода)
  * На второй, третьей и четвертой хост-системе - Worker node (рабочая нода) 
3. Для обеспечения отказоустойчивости На каждой рабочей ноде используем: 
  * один под с БД
  * один под с БД
  * два пода с Фронтенд
  * четыре пода с Бекенд
  
##### Таблица 2. Распределение подов систем на различных нодах
Система|Нода 1, шт.|Нода 2, шт.|Нода 3, шт.|Нода 4, шт.
|-|:-:|:-:|:-:|:-:|
БД|1|1|1||
Кэш|1|1|1||
Фронтенд|2|2|1||
...
Прикладное ПО|20
**Итоги рассчета, Гб**|**292.5**|**49**|**49,375**




1. Расчет дисковых ресурсов для одной хост-системы с одной рабочей нодой
##### Таблица 6. Количество дисковых ресурсов хост-системы
Система|Размер HDD, Гб
|-|:-:|
Рабочая нода|232,5
Операционная система|40
Прикладное ПО|20
**Итоги рассчета, Гб**|**292.5**

2. Рассчет количества CPU для одной хост-системы с одной рабочей нодой
##### Таблица 7. Количество CPU для одной хост-системы
Система|Кол-во CPU, шт.
|-|:-:|
Рабочая нода|45
Операционная система|2
Прикладное ПО|2
**Итоги рассчета, шт.**|**49**

3. Рассчет количества ОЗУ для одной хост-системы с одной рабочей нодой 
##### Таблица 8. Количество ОЗУ для одной хост-системы 
Система|Кол-во ОЗУ, Гб
|-|:-:|
Рабочая нода|45,375
Операционная система|2
Прикладное ПО|2
**Итоги рассчета, Гб**|**49,375**
