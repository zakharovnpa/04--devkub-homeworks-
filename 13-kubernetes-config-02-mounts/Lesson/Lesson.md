...
##### Команды создания файлов в контейнере
```
kubectl -n prod exec b-pod-0 -- sh -c "echo '49' > /static/49.txt"
```

##### Команды просмотра наличия файлов в контейнере
```
kubectl -n prod exec b-pod-0 -c backend -it bash -- ls static 
```

##### Команды просмотра содержимого файлов в контейнере
```
kubectl -n prod exec b-pod-0 -- sh -c "cat /static/49.txt"
```
...
##### Логи выполнения
- [Логи выполнения тестового создания окружения с подключением к общему тому в окружении prod](/13-kubernetes-config-02-mounts/Logs/logs-prod-backend-pvc-sc-pv.md)
