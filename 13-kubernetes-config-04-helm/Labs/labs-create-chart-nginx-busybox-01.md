## ЛР-1. Создание чарта на основе Deployment Nging + BusyBox
...
### 2. ДЗ вопрос 1. Подготовить helm чарт для приложения
...
#### Используемый манифест Deployment для создания из него чарта
* Deployment.yaml
...
spec:
  ...
  volumes:
    - name: my-volume
      emptyDir: {}
```
#### Шаблон для Deployment
* Tapmplates Deployment.yaml
```yml
apiVersion: apps/v1
kind: Deployment
metadata:
...
```
#### Создание чарта
* В директории чарта есть такие файлы:
...
#### Пример наполнения шаблона данными из манифеста

С данным шаблоном должны быть приложены следующие файлы:
* Values. yaml
```yml
...
##### Создаем файл из шаблона Chart.yaml
* Chart.yaml
```
apiVersion: v2
name: Chart-test-1
...
```
##### Создаем файл /templatetes/Deployment.yaml на основе нашего шаблона
* Deployment.yaml
