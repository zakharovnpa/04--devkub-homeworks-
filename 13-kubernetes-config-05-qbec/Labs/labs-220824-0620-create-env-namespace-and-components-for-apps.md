...
```
Компоненты загружаются из каталога компонентов, определенного для вашего приложения. 
По умолчанию это components/. Находится в исходном дереве, но вы можете изменить этот путь в qbec.yaml.

Компонент – это:

Один исходный файл jsonnet, json или yaml непосредственно в каталоге компонентов. 
В этом случае имя компонента — это имя файла без расширения, и qbec будет обрабатывать этот файл.

Подкаталог непосредственно в каталоге компонентов, в котором есть index.jsonnetфайл. 
В этом случае имя компонента является именем подкаталога и index.jsonnetобрабатывается qbec.

Подкаталог, содержащий index.yamlфайл. 
В этом случае имя компонента является именем подкаталога, и все файлы json и yaml в каталоге загружаются qbec.

Компонент может возвращать пустой набор объектов, если параметры среды выполнения определяют, что для конкретной 
целевой среды ничего не нужно устанавливать.
```
